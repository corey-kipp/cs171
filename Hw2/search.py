...
def best_first_graph_search(problem, f):
    ...
    while frontier:
        node = frontier.pop()
        if problem.goal_test(node.state):
            #print("Memory usage: ", max_frontier+max_explored)
            #print("Number of nodes expanded: ", max_explored)
            return node, max_frontier, max_explored
        explored.add(node.state)
        max_explored += 1
        for child in node.expand(problem):
            if child.state not in explored and child not in frontier:
                ...
                if f(child) < f(incumbent):
                    ...
                    frontier.append(child)
    return None, max_frontier, max_explored
...
def recursive_best_first_search(problem, h=None):
    ...
    h = memoize(h or problem.h, 'h')
    mem_track = [0, 0]
    def RBFS(problem, node, flimit, mt):
        if problem.goal_test(node.state):
            return node, 0, mt  # (The second value is immaterial)
        successors = node.expand(problem)
        mt[1] += 1
        mt[0] = max(len(node.path()), mt[0])
        if len(successors) == 0:
            return None, infinity, mt
        for s in successors:
        ...
        while True:
            ...
            best = successors[0]
            if best.f > flimit:
                return None, best.f, mt
            if len(successors) > 1:
                alternative = successors[1].f
            else:
                alternative = infinity
            result, best.f, mt = RBFS(problem, best, min(flimit, alternative), mt)
            if result is not None:
                return result, best.f, mt

    node = Node(problem.initial)
    node.f = h(node)
    result, bestf, mem_track = RBFS(problem, node, infinity, mem_track)
    return result, mem_track[0], mem_track[1]
